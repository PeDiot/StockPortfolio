...
def pred_for_each_data():
    ...
    with open(tmp + "df_temp.csv", "r") as csvfile:
    ...
    d.to_csv(tmp + "predictions.csv", index=False)
    dd = pd.DataFrame(liste_der_val)
    dd.to_csv(tmp + "last_predictions.csv", index=False)
...
def mean_value():
    ...
    liste_pred = list()
    with open(tmp + "predictions.csv", "r") as csvfile:
      reader = csv.reader(csvfile)
      next(reader)
      for elem in reader:
    ...
    liste_der_val = list()
    with open(tmp + "last_predictions.csv", "r") as csvfile:
      reader = csv.reader(csvfile)
      next(reader)
      for elem in reader:
    ...
    dd.to_csv(tmp + "portfolio_prediction.csv", index=False)
